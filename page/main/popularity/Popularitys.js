...
import { GET_POP_VIDEO, SAVE_LIKE } from './Query';
        ...
        <>
          <ScrollView style={{ flex: 1, width: '100%',paddingBottom:10 }}>
            <View style={{margin:10 }}>
              <Text style={{ textAlign:"center",fontWeight: 'bold', fontSize: 20 }}>인기 동영상</Text>
            </View>
            {data?.getPopVideo ?data?.getPopVideo?.map((item, index) => (
              <VideoView  key={index}>
...
export default Popularitys;
const PopularitysView = styled.View`
